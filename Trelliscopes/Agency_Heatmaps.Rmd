...
#political affiliation data by county
poll_data <- read.table('/Users/Becca/Downloads/countypres_2000-2020.tab', sep = '\t', header = TRUE)

#demographics of each county
```
...
test <- testdata[!is.na(testdata$name_department_award), ] #takes all of the NAs out of the column name_department_award

trel_dep <- test[,c("name_department_award", "zipcode_company", "name_agency_award", "amount_award", "state_company", "year_budget")] #select the columns that you want

names(trel_dep)[1]<- "Department" #rename name_department_award to Department to simplify

trel_dep <- nest(trel_dep, data = !c("Department")) #nest all the other columns into data except for Department

#example_data <- trel_dep[1,2][[1]][[1]] #look at the tibble of a department

testA <- testdata %>%
    distinct(year_budget) #find every unique year in year_budget
  
testB <- testdata %>%
    distinct(name_agency_award) #find every unique agency in name_agency_award
  
agency_heat_map_plot <- function(data){
  test <- data
  
  dataframeA <- data.frame() #make dataframeA a dataframe
  ...
  #rename these columns so they look pretty on the graph
  ...
  pl <- ggplot() +
   ...
   # add_text(text_position = 'top right')

  #ggplotly makes theh graph interactive
...
#make the trelliscope
trel_dept %>% 
  trelliscope(name = "Agency Heat Map Plots", nrow = 2, ncol = 2,
              path = '/Users/Becca/Documents/Data Consulting/sheldon_trelliscope_WI22/PersonalFolders/Becca Ebersole') #path = puts all of the trelliscopes that we make into one place so we can switch between them easily
```
